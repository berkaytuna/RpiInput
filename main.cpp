...
"Fieber Measurement: <input type=\"text\" name=\"input_2\" /> <br/>"
...
"Device Number: <input type=\"text\" name=\"input_6\" /> <br/>"
"IP 1: <input type=\"text\" name=\"input_7\" /> <br/>"
"IP 2: <input type=\"text\" name=\"input_8\" /> <br/>"
"IP 3: <input type=\"text\" name=\"input_9\" /> <br/>"
"IP 4: <input type=\"text\" name=\"input_10\" /> <br/>"
"IP 5: <input type=\"text\" name=\"input_11\" /> <br/>"
"IP 6: <input type=\"text\" name=\"input_12\" /> <br/>"
"IP 7: <input type=\"text\" name=\"input_13\" /> <br/>"
"IP 8: <input type=\"text\" name=\"input_14\" /> <br/>"
"IP 9: <input type=\"text\" name=\"input_15\" /> <br/>"
"IP 10: <input type=\"text\" name=\"input_16\" /> <br/>"
"<input type=\"submit\" />"
"</form></body></html>";
...
static int begin_request_handler(struct mg_connection* conn)
{
    ...
    char post_data[1024], serverIp[sizeof(post_data)], fev[sizeof(post_data)], san[sizeof(post_data)], delayt[sizeof(post_data)], 
		readt[sizeof(post_data)], devNum[sizeof(post_data)], ip1[sizeof(post_data)], ip2[sizeof(post_data)], ip3[sizeof(post_data)], ip4[sizeof(post_data)],
		ip5[sizeof(post_data)], ip6[sizeof(post_data)], ip7[sizeof(post_data)], ip8[sizeof(post_data)], ip9[sizeof(post_data)],
		ip10[sizeof(post_data)];
    int post_data_len;
    ...
    if (!strcmp(ri->local_uri, "/handle_post_request")) {
		...
		mg_get_var(post_data, post_data_len, "input_6", devNum, sizeof(readt));
		mg_get_var(post_data, post_data_len, "input_7", ip1, sizeof(ip1));
		mg_get_var(post_data, post_data_len, "input_8", ip2, sizeof(ip2));
		mg_get_var(post_data, post_data_len, "input_9", ip3, sizeof(ip3));
		mg_get_var(post_data, post_data_len, "input_10", ip4, sizeof(ip4));
		mg_get_var(post_data, post_data_len, "input_11", ip5, sizeof(ip5));
		mg_get_var(post_data, post_data_len, "input_12", ip6, sizeof(ip6));
		mg_get_var(post_data, post_data_len, "input_13", ip7, sizeof(ip7));
		mg_get_var(post_data, post_data_len, "input_14", ip8, sizeof(ip8));
		mg_get_var(post_data, post_data_len, "input_15", ip9, sizeof(ip9));
		mg_get_var(post_data, post_data_len, "input_16", ip10, sizeof(ip10));
		...
		while (1)
		{
			vector <char*> dataArray = { serverIp, fev, san, delayt, readt, devNum, ip1, ip2, ip3, ip4, ip5, ip6, ip7, ip8, ip9, ip10 };
...
int main()
{
	...
	*/
	...
	while (1)
	{
		...
		// Clear ip array in server
		char clearCmd[1] = { 0xCC };
		int sendResult = send(serverSocket, clearCmd, 1, 0);
		printf("sendResult 0xCC: %d\n", sendResult);
		delayfor(500);

		int devNum = stoi(settings[5]);
		//string ipStr = settingsStr[7];
		//char* ip = settings[7];
		//cout << "ipStr is: " << ipStr << endl;
		//cout << "ip is: " << ip << endl;
		//cout << "ipStr.length is: " << ipStr.length() << endl;
		for (int i = DEVNUM_POS; i < DEVNUM_POS + devNum; i++) {
			sendResult = send(serverSocket, settings[i], settingsStr[i].length(), 0);
			printf("sendResult ip: %d\n", sendResult);
			delayfor(500);
		}
		...
		while (1) {
			...
			if (bSend == true) {
				...
				sendResult = send(serverSocket, commandt, 1, 0);
				printf("sendResult 0xAA: %d\n", sendResult);
				int delayTime = stoi(settings[3]);
				delayfor(delayTime);
				sendResult = send(serverSocket, commandf, 1, 0);
				printf("sendResult 0xBB: %d\n", sendResult);
			}
			else
				delayfor(dt);
		}
